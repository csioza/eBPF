...
`sudo yum install libbpf-static.x86_64 `

`elfutils-libelf-devel-static.x86_64 zlib-static.x86_64 glibc-static.x86_64 -y`
...
# 四、挂载
1. shell将bpf内核态程序加载到网卡eth0

`ip link set dev eth0 xdp obj xdp-drop-world.o sec xdp`

2. 即刻时间中，在容器中的虚拟网卡上，只支持以通用模式挂载（xdpgeneric）
